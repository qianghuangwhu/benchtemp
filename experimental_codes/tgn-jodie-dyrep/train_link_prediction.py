...
model_name = "default"
if args.memory_updater == "gru":
    model_name = "TGN"
if args.embedding_module == "time":
    model_name = "Jodie"
if args.dyrep:
    model_name = "DyRep"
args.prefix = model_name
print(args.prefix + '-' + args.data)
...
fh = logging.FileHandler('log/{}.log'.format(args.prefix + '-' + args.data + '-LP-' + str(time.time())))
...
logger.info(args.prefix + '-' + args.data)
logger.info(args)
...
logger.info(node_features.shape)
logger.info(edge_features.shape)
logger.info('-------------------dataset information-------------------')
logger.info("dataset -------------------------------- interactions: {}, nodes: {}".format(
    full_data.n_interactions, full_data.n_unique_nodes))
logger.info("training dataset ----------------------- interactions: {}, nodes: {}".format(
    train_data.n_interactions, train_data.n_unique_nodes))
logger.info("validation dataset --------------------- interactions: {}, nodes: {}".format(
    val_data.n_interactions, val_data.n_unique_nodes))
logger.info("[Transductive] test dataset ------------ interactions: {}, nodes: {}".format(
    test_data.n_interactions, test_data.n_unique_nodes))

logger.info("[Inductive] ---------------------------- unseen nodes: {}".format(
    unseen_nodes_num))

logger.info("[Inductive new-]    validation dataset - interactions: {}, nodes: {}".format(
    new_node_val_data.n_interactions, new_node_val_data.n_unique_nodes))
logger.info("[Inductive new-]    test dataset ------- interactions: {}, nodes: {}".format(
    new_node_test_data.n_interactions, new_node_test_data.n_unique_nodes))

logger.info("[Inductive new-old] validation dataset - interactions: {}, nodes: {}".format(
    new_old_node_val_data.n_interactions, new_old_node_val_data.n_unique_nodes))
logger.info("[Inductive new-old] test dataset ------- interactions: {}, nodes: {}".format(
    new_old_node_test_data.n_interactions, new_old_node_test_data.n_unique_nodes))

logger.info("[Inductive new-new] validation dataset - interactions: {}, nodes: {}".format(
    new_new_node_val_data.n_interactions, new_new_node_val_data.n_unique_nodes))
logger.info("[Inductive new-new] test dataset ------  interactions: {}, nodes: {}".format(
...
val_rand_sampler = bt.lp.RandEdgeSampler(full_data.sources, full_data.destinations, seed=0)
nn_val_rand_sampler = bt.lp.RandEdgeSampler(new_node_val_data.sources, new_node_val_data.destinations,
                                            seed=1)
test_rand_sampler = bt.lp.RandEdgeSampler(full_data.sources, full_data.destinations, seed=2)
nn_test_rand_sampler = bt.lp.RandEdgeSampler(new_node_test_data.sources,
                                             new_node_test_data.destinations,
                                             seed=3)
new_old_test_rand_sampler = bt.lp.RandEdgeSampler(new_old_node_test_data.sources,
                                                  new_old_node_test_data.destinations,
                                                  seed=4)
new_new_test_rand_sampler = bt.lp.RandEdgeSampler(new_new_node_test_data.sources,
                                                  new_new_node_test_data.destinations,
                                                  seed=5)
...
#                                        new_node_test_data.destinations,
#                                        seed=3)
# new_old_test_rand_sampler = RandEdgeSampler(new_old_node_test_data.sources,
#                                             new_old_node_test_data.destinations,
#                                             seed=4)
# new_new_test_rand_sampler = RandEdgeSampler(new_new_node_test_data.sources,
#                                             new_new_node_test_data.destinations,
#                                             seed=5)
...
for i in range(args.n_runs):
    results_path = "results/{}_{}.pkl".format(args.prefix + "-" + args.data, i) if i > 0 else "results/{}.pkl".format(
    ...
    for epoch in range(NUM_EPOCH):
        ...
        # new-old validation on unseen nodes
        # new_old_val_ap, new_old_val_auc = eval_edge_prediction(model=tgn,
        #                                                        negative_edge_sampler=val_rand_sampler,
        #                                                        data=new_old_node_val_data,
        #                                                        n_neighbors=NUM_NEIGHBORS)
        #
        # # new-new validation on unseen nodes
        # new_new_val_ap, new_new_val_auc = eval_edge_prediction(model=tgn,
        #                                                        negative_edge_sampler=val_rand_sampler,
        ...
        epoch_mean_loss_list.append(np.mean(m_loss).astype(float))
        logger.info(
    ...
    new_old_test_ap, new_old_test_auc = eval_edge_prediction(model=tgn,
                                                             negative_edge_sampler=new_old_test_rand_sampler,
    ...
    new_new_test_ap, new_new_test_auc = eval_edge_prediction(model=tgn,
                                                             negative_edge_sampler=new_new_test_rand_sampler,
                                                             data=new_new_node_test_data,
                                                             n_neighbors=NUM_NEIGHBORS)
    logger.info("epoch mean losslist: %s", epoch_mean_loss_list)
    logger.info(
