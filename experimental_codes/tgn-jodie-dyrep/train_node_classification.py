...
fh = logging.FileHandler('log/{}.log'.format(args.prefix + '-' + args.data + '-NC-' + str(time.time())))
...
for i in range(args.n_runs):
    ...
    for epoch in range(args.n_epoch):
        ...
        val_auc, _ = eval_node_classification(tgn, decoder, val_data, full_data.edge_idxs, BATCH_SIZE,
                                              n_neighbors=NUM_NEIGHBORS)
        ...
        logger.info(
            f'Epoch {epoch}: train loss: {loss / num_batch}, val auc: {val_auc}, time: {time.time() - start_epoch}s')
    ...
    if args.use_validation:
        ...
        test_auc, test_ap = eval_node_classification(tgn, decoder, test_data, full_data.edge_idxs, BATCH_SIZE,
                                                     n_neighbors=NUM_NEIGHBORS)
    else:
    ...
    logger.info(f'test ap: {test_ap}')

    test_auc_list.append(test_auc)
    test_ap_list.append(test_ap)

logger.info('NC task -- auc: {} \u00B1 {}, ap: {} \u00B1 {}'.format(np.average(test_auc_list), np.std(test_auc_list),
                                                                    np.average(test_ap_list), np.std(test_ap_list)))
logger.info("--------------Rounding to four decimal places--------------")
logger.info('NC task -- auc: {} \u00B1 {}, ap: {} \u00B1 {}'.format(np.around(np.average(test_auc_list), 4),
                                                                    np.around(np.std(test_auc_list), 4),
                                                                    np.around(np.average(test_ap_list), 4),
                                                                    np.around(np.std(test_ap_list), 4)))