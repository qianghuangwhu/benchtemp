...
if args.mode == 't':
    ...
    # new-
    valid_test_new_flag = np.logical_or(valid_test_new_new_flag, valid_test_new_old_flag)
...
if args.mode == 'i':
    ...
    test_src_new_l, test_dst_new_l, test_ts_new_l, test_e_idx_new_l, test_label_new_l = \
        src_l[valid_test_new_flag], dst_l[valid_test_new_flag], ts_l[valid_test_new_flag], e_idx_l[valid_test_new_flag], label_l[valid_test_new_flag]
...
for i in range(args.n_runs):
    ...
    logger.info('Transductive: Test statistics: : {} all nodes -- acc: {}, auc: {}, ap: {}'.format(args.mode, test_acc, test_auc, test_ap))
    test_new_new_acc, test_new_new_ap, test_new_new_auc, test_new_old_acc, test_new_old_ap, test_new_old_auc = [-1]*6
    if args.mode == 'i':
        test_new_acc, test_new_ap, test_new_f1, test_new_auc = eval_one_epoch('test for {} nodes'.format(args.mode), cawn, test_rand_sampler, test_src_new_l, test_dst_new_l,test_ts_new_l, test_label_new_l, test_e_idx_new_l)
        logger.info('Inductive: Test statistics: {} new- nodes ------ acc: {}, auc: {}, ap: {}'.format(args.mode, test_new_acc,test_new_auc,test_new_ap))
        ...
        logger.info('Inductive: Test statistics: {} new-new nodes --- acc: {}, auc: {}, ap: {}'.format(args.mode, test_new_new_acc, test_new_new_auc,test_new_new_ap ))
        ...
        logger.info('Inductive: Test statistics: {} new-old nodes --- acc: {}, auc: {}, ap: {}'.format(args.mode, test_new_old_acc, test_new_old_auc, test_new_old_ap))
