...
def get_args():
    ...
    # 'sx-askubuntu', 'sx-superuser', 'wiki-talk-temporal', 'mooc'],
    ...
    parser.add_argument('--bias', default=0.0, type=float,
                        help='the hyperparameter alpha controlling sampling preference with time closeness, default to 0 which is uniform sampling')
    ...
    parser.add_argument('--linear_out', action='store_true', default=False,
                        help="whether to linearly project each node's ")
    ...
    parser.add_argument('--replace_prob', type=float, default=0.9,
                        help='probability for storing new neighbors to N-cache replacing old ones')
    ...
    parser.add_argument('--n_runs', type=int, default=2, help='number of model runs')
