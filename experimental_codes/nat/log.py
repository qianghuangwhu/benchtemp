...
def set_up_logger(args, sys_argv):
    ...
    runtime_id = '{}-{}-{}-{}-{}-{}-{}'.format("NAT", args.data, str(time.time()), args.mode[0], n_hop,
                                               'k'.join(n_degree), args.pos_dim)
...
def set_up_logger_nc(args, sys_argv):
    ...
    n_degree, n_hop = process_sampling_numbers(args.n_degree, args.n_hop)
    n_degree = [str(n) for n in n_degree]
    runtime_id = '{}-NC-{}-{}-{}-{}-{}-{}'.format("NAT", args.data, str(time.time()), args.mode[0], n_hop,
                                               'k'.join(n_degree), args.pos_dim)
    logging.basicConfig(level=logging.INFO)
    logger = logging.getLogger()
    logger.setLevel(logging.DEBUG)
...
def save_oneline_result(dir, args, test_results):
    ...
    with open(dir + 'oneline_results.txt', 'a') as f:
        elements = [str(e) for e in [args.data, args.mode[0], n_hop, 'k'.join(n_degree), args.pos_dim,
