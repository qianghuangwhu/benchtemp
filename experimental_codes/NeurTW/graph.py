        ...
        for i, (src_idx, cut_time) in enumerate(zip(src_idx_l, cut_time_l)):
            ...
            if ngh_binomial_prob is None:
                ...
                # spatiotemporal sampling with exploration and exploitation trade-off
                    ...
                    temperal_sampling_weight = - self.temporal_bias * time_delta
                    for index in range(len(temperal_sampling_weight)):
                        if temperal_sampling_weight[index] == 0:
                            temperal_sampling_weight[index] = 1
                        if temperal_sampling_weight[index] < 0:
                            temperal_sampling_weight[index] = - 1 / temperal_sampling_weight[index]
                            
                    temperal_sampling_weight = np.array(temperal_sampling_weight)
                    # temperal_sampling_weight = np.exp(
                    #    - self.temporal_bias * time_delta)

                    # if temperal_sampling_weight.sum() == 0:
                    #     temperal_sampling_weight = np.random.random_sample(temperal_sampling_weight.shape)
                    ...
                    for _ in range(num_neighbor):
                        ...
                        if np.isnan(sampling_weight).all():
                            sampling_weight = np.random.random_sample(sampling_weight.shape)
                            sampling_weight /= sampling_weight.sum()
                        else:
                            sampling_weight[np.isnan(sampling_weight)] = 0.0
                            sampling_weight /= sampling_weight.sum()
