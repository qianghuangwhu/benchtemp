...
def set_up_logger(args, sys_argv):
    ...
    runtime_id = '{}-{}-{}-{}-{}-{}'.format(args.mode[0], args.data, str(time.time()),
                                            n_layer, 'k'.join(n_degree), args.pos_dim)
...
def set_up_logger_nc(args, sys_argv):
    # set up running log
    n_degree, n_layer = process_sampling_numbers(args.n_degree, args.n_layer)
    n_degree = [str(n) for n in n_degree]
    runtime_id = '{}-NC-{}-{}-{}-{}-{}'.format(args.mode[0], args.data, str(time.time()),
                                            n_layer, 'k'.join(n_degree), args.pos_dim)
    logging.basicConfig(level=logging.INFO)
    logger = logging.getLogger()
    logger.setLevel(logging.DEBUG)
...
def save_oneline_result(dir, args, test_results):
    ...
    with open(dir+'oneline_results.txt', 'a') as f:
        elements = [str(e) for e in [args.data, args.mode[0], n_layer, 'k'.join(n_degree), args.pos_enc, args.pos_dim,
