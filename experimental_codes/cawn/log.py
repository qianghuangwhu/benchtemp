...
def set_up_logger(args, sys_argv):
    ...
    runtime_id = '{}-{}-{}-{}-{}-{}-{}'.format(args.mode[0], args.data, str(time.time()), args.agg, n_layer, 'k'.join(n_degree), args.pos_dim)
...
def set_up_logger_NC(args, sys_argv):
    ...
    n_degree, n_layer = process_sampling_numbers(args.n_degree, args.n_layer)
    n_degree = [str(n) for n in n_degree]
    runtime_id = '{}-NC-{}-{}-{}-{}-{}-{}'.format(args.mode[0], args.data, str(time.time()), args.agg, n_layer, 'k'.join(n_degree), args.pos_dim)
    logging.basicConfig(level=logging.INFO)
    logger = logging.getLogger()
    logger.setLevel(logging.DEBUG)
    file_path = 'log/{}.log'.format(runtime_id) #TODO: improve log name
