...
def eval_one_epoch_nc(hint, tgan, sampler, src, dst, ts, label, val_e_idx_l=None):
    # val_acc, val_ap, val_f1, val_auc = [], [], [], []
    pred_prob = np.zeros(len(label))
    with torch.no_grad():
        tgan = tgan.eval()
        TEST_BATCH_SIZE = 30
        num_test_instance = len(src)
        ...
        for k in range(num_test_batch):
            ...
            pos_prob, _ = tgan.contrast(src_l_cut, dst_l_cut, dst_l_fake, ts_l_cut, e_l_cut, test=True)

            pred_prob[s_idx: e_idx] = pos_prob.cpu().numpy()
    try:
        auc_roc = roc_auc_score(label, pred_prob)
    except ValueError:
        return 1
    return auc_roc
